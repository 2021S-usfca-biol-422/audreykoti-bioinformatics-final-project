...
vcf_with_metadata %>%
  ...
  labs(title = "COVID-19 Patient Demographics (Dec 2020 to Jan 2021)",
       x = "Age") +
  theme_few()
...
# mostly men infected (more interactions at work). ages 30 - 50 but most men
...
```{r s-gene-snps-by-age-plo}
# plot the occurrence of variation in the S gene by age
vcf_with_metadata %>%
  ...
  ggplot(aes(x = Host_Age,
             ...
             fill = host_sex)) +
  geom_col(position = position_dodge()) +
  labs(title = "Occurrence of the S Gene Variation by Age",
       x = "Age",
       y = "Count") +
  theme_few()
```

**Figure 3**: Figure 3 shows the count of the S gene variations in patients older than 18 years.
# S variations more common in males (increased interactions at work)
...
**Figure 4**: Figure 4 shows the distribution of SNPs in each sex.

```{r s-gene-snps-by-age-plot}
# plot the unique alternative alleles in the S gene by age


```

**Figure 5**: Figure 5 counts the number of unique alternative alleles in the S gene to determine the most common one in patients of different ages.

```{r s-gene-snps-by-sex-plot}
# plot the unique alternative alleles in the S gene by sex

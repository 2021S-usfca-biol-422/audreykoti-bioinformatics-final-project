...
vcf_with_metadata %>%
  ...
  labs(title = "Normalized occurrence of the S Gene Variants by Date",
       x = "Date",
       y = "Count") +
  theme_few()
```

**Figure 5**: Figure 5 shows the occurrence of S gene variants each day between December 2020 and January 2021.
...
vcf_with_metadata %>%
  ...
  group_by(gene, pos, alt, ref, Host_Age, host_sex) %>%
  tally() %>%
  group_by(gene, alt, ref, Host_Age, host_sex) %>%
  tally() %>%
  filter(gene == "S", alt == "T") %>%
  ggplot(aes(x = ref,
             y = n,
             fill = host_sex)) +
  geom_col(position = position_dodge()) +
  labs(title = "Distribution of the T Allele by Reference Allele and Sex",
       x = "Reference Allele",
       y = "Count") +
  theme_few() +
...
**Figure 6**: Figure 6 shows the change to the "T" allele from each reference allele in the S gene by host age and sex.
